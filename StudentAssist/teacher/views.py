...
def home(request):
    ...
    courses = Course.objects.filter(teacher=teacher)    #for filtering the resp course that is created by a particular teacher

    return render(request, 'teacher.html', context={'teacher': teacher, 'courses': courses})    #for calling teacher.html
...
def selectlinks(request, lecture_pk):
    ...
    if (request.method == 'GET'):
        ...
        try:
            ...
            teacher = get_object_or_404(Teacher, user=request.user)
            some_var = request.POST.getlist('checks[]')             #for selecting the links 
            for i in some_var:
                link=Links(teacher=teacher, lecture=lecture, links=i)
                link.save()
            return redirect(viewcourse, lecture.course.id)          #after selecting the links, we'll be redirected to view course page
        except ValueError:
...
def sendmail(to):
    ...
    server.starttls()
    server.login('abc@gmail.com', 'Strong@123')
    server.sendmail('strong@gmail.com', to, content)
    server.close()
